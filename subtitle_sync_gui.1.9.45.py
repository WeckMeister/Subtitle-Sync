...
__version__ = __versionMinor__  + "45"
import os
import sys
import time
import wave
...
class SubtitleSyncApp:
    ...
    def run_sync(self):
        ...
        # Attach handler to faster_whisper logger
        ...
        # Optional cleanup at the end of run_sync()
        self.gui_handler = gui_handler  # Save reference so you can remove it later
        ...
        beam = self.beam_size.get()
        tag = f"ASR{beam:02}" + ("1W" if self.word_level_asr.get() else "")
        ...
        with open(transcription_path, "w", encoding="utf-8") as final_file, \
            ...
            for i, segment in enumerate(segment_gen):
                ...
                text = segment.text.strip() or (
                    " ".join(w.word for w in getattr(segment, "words", [])) if hasattr(segment, "words") else ""
                )
                if not text:
                    self.debug("[WARN] Empty segment {} — skipping.", i + 1)
                    continue

                entry = f"{i+1}\n{self.format_timestamp(segment.start)} --> {self.format_timestamp(segment.end)}\n{text}\n\n"
                ...
                self.root.update_idletasks()
                time.sleep(0.01)  # Let the GUI catch its breath
    ...
    def run_asr_only(self):
        ...
        with open(asr_path, "w", encoding="utf-8") as f:
            ...
            for segment in segment_gen:
                ...
                if not text:
                    self.debug("[WARN] Empty segment {} — skipping.", line_count + 1)
                    continue
                ...
                self.right_tree.insert(
                    "", "end",
                    values=(line_count, f"{start} --> {end}", text)
                )
                if self.auto_scroll_right:
                    self.right_tree.yview_moveto(1.0)
