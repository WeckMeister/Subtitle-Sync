...
__version__ = __versionMinor__  + "66"
import os
import sys
import time
import wave
...
class ModelDownloader(tk.Toplevel):
    ...
    def ensure_model(model_dir="models/whisper-large-v3"):
        ...
        prompt = messagebox.askyesnocancel(
            ...
            "Yes = Browse for model folder\n"
            "No = Download it now (~3GB)\n"
            "Cancel = Exit app"
        )
        ...
        if prompt is None:
            ...
            chosen_dir = filedialog.askdirectory(title="Select Whisper Model Folder")
            if chosen_dir and os.path.exists(chosen_dir):
                return chosen_dir
            else:
                messagebox.showerror("Invalid Folder", "The selected folder doesn't contain the model.")
                sys.exit()
        else:
            # Fallback to your current download logic
            win = ModelDownloader(tk.Toplevel(), "openai/whisper-large-v3", model_dir)
            win.master.withdraw()
...
class SubtitleSyncApp:
    ...
    def run_sync(self):
        ...
        # 🔐 Ensure model is available before anything else
        model_path = ModelDownloader.ensure_model()

        # 🛑 Alert user if huggingface_hub is outdated
        if version.parse(hf_version) < version.parse("0.20.0"):
            messagebox.showwarning(
                "Update Recommended",
                "Your version of 'huggingface_hub' is older than 0.20.0.\n"
        ...
        # ... [rest of your transcription loop remains unchanged]
