    ...
    def merge_subtitles(self, original_lines, asr_lines):
        ...
        for index, orig in enumerate(original_blocks, 1):
            ...
            for asr in asr_blocks:
                delta = abs(to_seconds(asr["start"]) - orig_start)
                if delta < min_time_diff and delta <= threshold:
                    min_time_diff = delta
                    best_match = asr

            # Optional fuzzy fallback (for very loose matches)
            if not best_match and asr_blocks:
                similarities = [
                    (difflib.SequenceMatcher(None, orig["text"], a["text"]).ratio(), a)
                    for a in asr_blocks
                ]
                best_match = max(similarities, key=lambda x: x[0])[1] if similarities else None
        ...
        self.feedback_label.config(
            text=f"🔗 Sync completed: {matched_count}/{len(original_blocks)} lines matched"
        )
