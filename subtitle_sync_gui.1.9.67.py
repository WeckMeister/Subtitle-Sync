...
__version__ = __versionMinor__  + "67"
import os
import sys
import time
import wave
...
def find_ffmpeg() -> str:
    """
    Return a path to ffmpeg.exe, preferring the bundled copy under bin/,
    falling back to the first ffmpeg on PATH, or raising if neither exists.
    """
    # 1) bundled in your app’s bin/ folder
    bundled = resource_path(os.path.join("bin", "ffmpeg.exe"))
    if os.path.isfile(bundled):
        return bundled

    # 2) system-wide ffmpeg
    system_ff = which("ffmpeg")
    if system_ff:
        return system_ff

    # neither found: fatal error
    raise FileNotFoundError(
        f"Could not locate ffmpeg.exe.\n"
        f"Tried bundled path:\n  {bundled}\nand your $PATH."
    )
...
class SubtitleSyncApp:
    ...
    def run_sync(self):
        ...
        try:
            ...
            self.debug("[INFO] Whisper model: %s", model_path)
            self.debug("[INFO] ffmpeg binary: %s", ffmpeg_path)

            # 2) Prep UI
            self.status_label.config(text="🔁 Extracting audio…", fg="black")
            self.feedback_label.config(text="🔁 Preparing audio for ASR…")
            ...
            # 4) Attach Whisper logger so you see progress in your console
            self.attach_whisper_logger()

            # 5) Transcribe
            self.feedback_label.config(text="💬 Transcribing…")
            ...
            # 6) Convert segments to a temporary ASR .srt
            temp_asr = tempfile.NamedTemporaryFile(suffix=".srt", delete=False, mode="w", encoding="utf-8")
            ...
            # 7) Merge subtitles
            self.feedback_label.config(text="🔗 Synchronizing subtitles…")
            ...
            with open(self.output_path.get(), "w", encoding="utf-8") as out_f:
            ...
            self.debug(f"[ERROR] run_sync() crashed:\n{tb}")
            self.root.after(0, lambda: self.feedback_label.config(
                text="❌ Sync failed — see log"))
    ...
    def run_asr_only(self):
        ...
        try:
            ...
            model_dir = "models/whisper-large-v3"
            model_path = ModelDownloader.ensure_model(repo_id, model_dir)
            self.debug("[INFO] Whisper model at %s", model_path)

            # 2) Find ffmpeg
            ffmpeg_path = find_ffmpeg()
            self.debug("[INFO] Using ffmpeg at %s", ffmpeg_path)

            # 3) Prepare UI
            self.status_label.config(text="Running ASR only…", fg="black")
            self.feedback_label.config(text="🎙️ Extracting audio…")
            ...
            # 5) Transcribe
            self.feedback_label.config(text="💬 Transcribing with Whisper…")
            self.status_label.config(text="Transcribing…")
            ...
            # 6) Write out a simple .srt (index / timestamps / text)
            ...
            tb = traceback.format_exc()
            self.debug(f"[ERROR] run_asr_only() crashed:\n{tb}")
            self.root.after(0, lambda: self.feedback_label.config(
                text="❌ ASR-only failed — see log"))
