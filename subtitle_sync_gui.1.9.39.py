...
__version__ = __versionMinor__  + "39"
import os
import sys
import time
import wave
...
class SubtitleSyncApp:
    ...
    def run_sync(self):
        ...
        self.debug("[DEBUG] Calling model.transcribe")

        t4 = time.time()
        segments = self.capture_transcribe_output(model, audio_path)
        try:
            segments = self.capture_transcribe_output(model, audio_path)
            self.debug("[DEBUG] Segments returned: {}", len(segments))
        except Exception as e:
            self.debug("[ERROR] Segment debug failed: {}", str(e))

        self.debug("[TEST] capture_transcribe_output about to run…")
        ...
        for s in segments[:5]:
            ...
            self.debug("▶ Segment: {} → {} | {}", self.format_timestamp(s.start), self.format_timestamp(s.end), snippet)
