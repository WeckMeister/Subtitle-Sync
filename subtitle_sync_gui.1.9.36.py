...
__version__ = __versionMinor__  + "36"
import os
import sys
import time
import wave
            ...
            def write(self, text):
                ...
                if "\n" in self.buffer:
                    ...
                    for line in lines[:-1]:
                        line = line.strip()
                        if line:
                            self.root.after(0, lambda l=line: self.log_func("[INFO] " + l))
                    self.buffer = lines[-1]

            def flush(self):
                if self.buffer.strip():
                    self.root.after(0, lambda: self.log_func("[INFO] " + self.buffer.strip()))
                self.buffer = ""

        old_stdout, old_stderr = sys.stdout, sys.stderr
        streamer = LogStreamer(self.root, self.log_feedback)
        sys.stdout = sys.stderr = streamer
