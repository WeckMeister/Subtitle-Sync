...
__version__ = __versionMinor__  + "35"
import os
import sys
import time
import wave
...
class SubtitleSyncApp:
    ...
    def capture_transcribe_output(self, model, audio_path):
        ...
        # Redirect stdout and stderr to capture Whisper logs
        with contextlib.redirect_stdout(buffer), contextlib.redirect_stderr(buffer):
            ...
            segments = list(segments_gen)  # 👈 Execute inside redirect block to capture logs

        raw_log = buffer.getvalue()
        if raw_log.strip():
            self.root.after(0, lambda: self.log_feedback("[INFO] Whisper internal log:\n" + raw_log.strip()))
