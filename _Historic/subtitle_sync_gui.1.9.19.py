...
__version__ = __versionMinor__  + "19"
...
class SubtitleSyncApp:
    ...
    "live_scroll": tk.PhotoImage(file="icons/live_scroll.png"),
    ...
    def run_sync(self):
        ...
        print(f"[DEBUG] Segments returned: {len(segments)}")  # check to see if someting pops up
        ...
        tag = "Word-Level" if self.word_level_asr.get() else "Sentence-Level"
        self.right_label.config(
            text=f"🧠 ASR Generated: {os.path.basename(transcription_path)} [{tag}]"
        )
    ...
    def merge_subtitles(self, original_lines, asr_lines):
        ...
        for orig in original_blocks:
            ...
            # Find closest ASR block by timestamp
            best = None
            best_diff = float("inf")

            for asr in asr_blocks:
                asr_time = to_seconds(asr["start"])
                if asr_time is None or orig_time is None:
                    continue
                diff = abs(asr_time - orig_time)
            ...
            if best:
                ...
                # No match found, keep original timing
                result.extend([
                    f"{index}",
                    f"{orig['start']} --> {orig['end']}",
                    orig_text,
        ...
        self.feedback_label.config(
            text=f"🔗 Realignment complete: {matched}/{len(original_blocks)} lines retimed"
        )
        return [line + "\n" for line in result]
    ...
    def run_asr_only(self):
        ...
        ### removed whisper action from here
        ...
        print(f"[DEBUG] Segments returned: {len(segments)}") #trying to find issue

        for s in segments[:5]:
            print(f"Text: {s.text.strip()} | Start: {s.start} | End: {s.end}")
        ...
        self.feedback_label.config(text=f"✅ ASR complete. Output saved to: {os.path.basename(asr_path)}")
        self.status_label.config(text="✔ ASR complete.", fg="#2e7d32", font=("Segoe UI", 9, "bold"))
        self.progress["value"] = 100
