...
__version__ = __versionMinor__  + "41"
import os
import sys
import time
import wave
...
class SubtitleSyncApp:
    ...
    def run_sync(self):
        ...
        if self.stop_flag.is_set():
            ...
            self.feedback_label.config(text="⚠️ Process halted before completion.")
            self.progress["value"] = 0
            self.debug("[INFO] Transcription process terminated early by Stop.")
            return
    ...
    def run_asr_only(self):
        ...
        with open(asr_path, "w", encoding="utf-8") as f:
            ...
            for segment in segment_gen:
                ...
                if not segment.text.strip() and hasattr(segment, "words") and segment.words:
                    text = " ".join(w.word for w in segment.words)
                else:
                    text = segment.text.strip()

                if not text:
                    self.debug("[WARN] Empty segment {} — skipping or writing placeholder.", line_count + 1)
                    continue
