...
__version__ = __versionMinor__  + "37"
import os
import sys
import time
import wave
...
class SubtitleSyncApp:
    ...
    def capture_transcribe_output(self, model, audio_path):
        ...
        # Get the faster_whisper logger
        whisper_logger = logging.getLogger("faster_whisper")
        whisper_logger.setLevel(logging.DEBUG)               # ← Force level
        whisper_logger.propagate = False                     # ← Prevent going to terminal

        gui_handler = GuiLogHandler(self)
        gui_handler.setFormatter(logging.Formatter("[%(levelname)s] %(message)s"))
        whisper_logger.addHandler(gui_handler)

        # Optional: remove other handlers (that write to stderr)
        for h in list(whisper_logger.handlers):
            if isinstance(h, logging.StreamHandler) and h is not gui_handler:
                whisper_logger.removeHandler(h)
        ...
        if raw:
            for line in raw.splitlines():
                self.root.after(0, lambda l=line: self.log_feedback(f"[INFO] {l.strip()}"))
