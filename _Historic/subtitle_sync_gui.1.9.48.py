...
class SubtitleSyncApp:
    ...
    def run_sync(self):
        ...
        with open(transcription_path, "w", encoding="utf-8") as final_file, \
            ...
            for segment in segment_gen:
                if self.stop_flag.is_set():
                    self.feedback_label.config(text="⛔️ Sync stopped by user.")
                    self.status_label.config(text="❌ Sync interrupted.", fg="red")
                    self.progress["value"] = 0
                    self.debug("[INFO] Stop was pressed. Sync interrupted.")
        ...
        if os.path.exists(transcription_path):
            ...
            self.feedback_label.config(text=f"✅ Sync complete: {os.path.basename(transcription_path)}")
        ...
        self.debug("[INFO] Sync complete. Output: {} ({} lines)", os.path.basename(transcription_path), line_count)
    ...
    def run_asr_only(self):
        ...
        with open(asr_path, "w", encoding="utf-8") as f:
            # Optional: Insert SRT comment with metadata
            # f.write(f"NOTE: Generated by Whisper | Beam Size: {beam} | Granularity: {granularity}\n\n")
