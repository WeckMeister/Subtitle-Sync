...
__version__ = __versionMinor__  + "04"
...
class SubtitleSyncApp:
    ...
    "sync_onlygreen": tk.PhotoImage(file="icons/sync.png"),  # reuse or custom icon
}
    ...
    def create_ribbon(self):
        ...
        btns = [
            ...
            ("Full ASR and Sync", self.start_process, "sync"),
            ("Sync Only", self.sync_only_mode, "sync_only"),  # new
    ...
    def update_status_bar(self):
        ...
        if self.full_sync_btn:
            ...
            icon_name = "syncgreen" if ready else "sync"
            self.full_sync_btn.config(image=self.icons[icon_name])

        # Update [Sync Only] button icon
        if self.sync_only_btn:
            ready = self.subtitle_path.get() and self.asr_path and self.output_path.get()
            icon_name = "sync_onlygreen" if ready else "sync_only"
            self.sync_only_btn.config(image=self.icons[icon_name])
