...
__version__ = __versionMinor__  + "48"
import os
import sys
import time
import wave
...
class SubtitleSyncApp:
    ...
    def create_ribbon(self):
        ...
        self.btn_open_video = ttk.Button(ribbon, text="Open Video", command=self.select_video)
        self.btn_open_video.pack(side="left", padx=4)

        self.btn_asr = ttk.Button(ribbon, text="ASR Only", command=self.start_asr_only, state="disabled")
        self.btn_asr.pack(side="left", padx=4)

        self.btn_sync = ttk.Button(ribbon, text="Full Sync", command=self.start_process, state="disabled")
        self.btn_sync.pack(side="left", padx=4)

        self.btn_asr.config(state="disabled")
    ...
    def select_subtitle(self):
        ...
        if path:
            ...
            self.debug("[INFO] Subtitle loaded: {}", os.path.basename(path))  # 👈 New log line
            ...
            if not self.output_path.get():
                ...
                self.debug("[INFO] Will export to file: {}", os.path.basename(out_path))
    ...
    def select_video(self):
        ...
        if self.video_path.get() and os.path.exists(self.video_path.get()):
            self.btn_asr.config(state="normal")
    ...
    def run_sync(self):
        ...
        if not os.path.exists(ffmpeg_path):
            raise FileNotFoundError("ffmpeg.exe not found at: {}".format(ffmpeg_path))
        ...
        self.debug("[DEBUG] Audio extracted in {:.2f}s", time.time() - t0)
        self.status_label.config(text="Transcribing with Faster-Whisper...", fg="black")
        ...
        self.debug("[DEBUG] Whisper model loaded: {}", model_path)

        total_duration = self.get_audio_duration(audio_path)
        start_time = time.time()

        base = os.path.splitext(os.path.basename(self.video_path.get()))[0]
        tag = f"ASR{self.beam_size.get():02}" + ("1W" if self.word_level_asr.get() else "")
        transcription_path = self.output_path.get()
        preview_path = os.path.join(os.path.dirname(transcription_path), f"{base}.preview.srt")
        whisper_path = os.path.join(os.path.dirname(transcription_path), f"{base}.{tag}.srt")
        ...
        with open(transcription_path, "w", encoding="utf-8") as final_file, \
            ...
            for segment in segment_gen:
                if self.stop_flag.is_set():
                    ...
                    self.status_label.config(text="❌ Transcription stopped.", fg="red")
                    self.progress["value"] = 0
                    self.debug("[INFO] Stop was pressed. Transcription loop interrupted.")
                ...
                if len(self.preview_buffer) >= self.flush_lines.get():
                    ...
                    self.preview_timestamp.config(text=f"Last updated: {time.strftime('%H:%M:%S')}")
        ...
        self.status_label.config(text="✔ Transcription complete.", fg="#2e7d32", font=("Segoe UI", 9, "bold"))
        ...
        self.right_label.config(text=f"🧠 ASR Generated: {os.path.basename(transcription_path)} [{tag_label}]")
    ...
    def run_asr_only(self):
        ...
        ffmpeg_path = resource_path(os.path.join("bin", "ffmpeg.exe"))
        if not os.path.exists(ffmpeg_path):
            raise FileNotFoundError("ffmpeg.exe not found at expected path.")
        ...
        self.debug("[INFO] Starting ASR-only transcription for video: {}", os.path.basename(self.video_path.get()))
        ...
        asr_path = os.path.join(os.path.dirname(self.video_path.get()), f"{base}.{tag}.srt")
        ...
        with open(asr_path, "w", encoding="utf-8") as f:
            ...
            for segment in segment_gen:
                if self.stop_flag.is_set():
                    self.feedback_label.config(text="⛔️ ASR-only stopped by user.")
                    self.status_label.config(text="❌ Transcription stopped.", fg="red")
                    self.progress["value"] = 0
                    self.debug("[INFO] Stop was pressed. Transcription loop interrupted.")
                    self.detach_whisper_logger()
                ...
                self.status_label.config(
                    text=f"Transcribing… {percent:.1f}% | ⏱ Elapsed: {time.strftime('%M:%S', time.gmtime(elapsed))} | ⌛ Remaining: {time.strftime('%M:%S', time.gmtime(remaining))}"
                )
                self.feedback_label.config(text=f"💬 Whisper processing: {percent:.1f}%")
                self.root.update_idletasks()
                time.sleep(0.01)
    ...
    def capture_transcribe_output(self, model, audio_path):
        ...
        class GuiLogHandler(logging.Handler):
            ...
            def emit(self, record):
                msg = self.format(record)
                self.app.root.after(0, lambda: self.app.log_feedback(f"[WHISPER] {msg}"))
        ...
        try:
            ...
            segment_gen, _ = model.transcribe(
                audio_path,
                beam_size=self.beam_size.get(),
                word_timestamps=True  # <--- This must be True for word-level details
            )

            for segment in segment_gen:
                segments.append(segment)
                percent = min((segment.end / total_duration) * 100, 100)
                self.root.after(0, lambda p=percent: self.progress.update_idletasks() or self.progress.config(value=p))
                self.root.after(0, lambda p=percent: self.status_label.config(text=f"Transcribing… {p:.1f}%"))
                self.root.after(0, lambda p=percent: self.feedback_label.config(text=f"💬 Whisper processing: {p:.1f}%"))

                # Optional: live preview in right pane (first 20 words)
                if hasattr(segment, "text"):
                    text = segment.text.strip()
                    if text:
                        timestamp = f"{self.format_timestamp(segment.start)} --> {self.format_timestamp(segment.end)}"
                        self.root.after(0, lambda t=timestamp, tx=text: self.right_tree.insert("", "end", values=(len(segments), t, tx[:80])))

        finally:
            whisper_logger.removeHandler(gui_handler)
